...
causing all the other stones to shift over a bit
...
-   If the stone is engraved with a number that has an even number of digits,
    it is replaced by two stones. The left half of the digits
...
If you have an arrangement of five stones
engraved with the numbers 0 1 10 99 999 and you blink once,
...
an arrangement of seven stones engraved with the numbers 1 2024 1 0 9 9 2021976.
...
def blink(*stones: int, n: int = 1) -> list[int]:
    for _ in range(n):
        stones = _update_stones(stones)
    return list(stones)


def _update_stones(stones: list[int]) -> Iterable[int]:
    for s in stones:
        if s == 0:
            yield 1
        elif _is_len_even(s):
...
def _is_len_even(n):
    return not bool(len(str(n)) % 2)


assert _is_len_even(1) is False
assert _is_len_even(10) is True
...
assert len(blink(125, 17, n=25)) == 55312

print(len(blink(0, 89741, 316108, 7641, 756, 9, 7832357, 91, n=25)))
